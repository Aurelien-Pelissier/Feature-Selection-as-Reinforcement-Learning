...
This repository contains the source code to perform feature selection with a reinforcement learning approach, where the feature set state space is represented by a Direct Acyclic Graph (DAG), the C++ implementation of two Monte Carlo DAG Search algorithms is gicen: The first one, FUSE (https://hal.inria.fr/inria-00484049/document), starts from the empty feature subset and relies on UCT to identify the best feature subest in the DAG with a fixed budget, while the second one, BLI-MCDS (https://docdro.id/e09o21k) solve the Best Arm Identification problem at different stage in the DAG until a stopping condition is verified.
