...
For the first ~10000 iterations, the algorithm kept exploring deeper, until it started to select stopping features and enventually explored at lower depth until it finally converged to depth = 3. One can not that each time the algorithm explored depth higher the 3, the reward significantly decreased.
